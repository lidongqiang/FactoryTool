	...
	case CDDS_ITEMPOSTPAINT:
		{
			...
			COLORREF crNewBK;
			BOOL bChangeColor;
			GetCellColors(hItem,&dc,&bChangeColor);

			crNewBK = dc.GetBkColor();
			...
			if (crTextBk != crNewBK)	// draw label's background
			{
				CRect rcSelect =  rcLabel;
				if(bFullRowSelect) rcSelect.right = rcItem.right;
				//UINT uiState;
				//uiState = m_Tree.GetItemState(hItem,TVIF_STATE);
				...
				if (!bChangeColor)
				{
					...
					dc.FillSolidRect(&rcSelect, crNewBK);
...
{
	CTVHITTESTINFO htinfo = {{pt.x,pt.y}, 0, NULL, 0};
...
void CColumnTreeCtrl::GetCellColors(HTREEITEM hItem, CDC *pDC,BOOL *pChangeFlag)
{
	TREEITEMCOLOR_MAP::iterator iter;
	m_csColorMap.Lock();
	iter = m_colorMap.find(hItem);
	*pChangeFlag = FALSE;
	if (iter!=m_colorMap.end())
	{
		*pChangeFlag = TRUE;
		pDC->SetBkColor(iter->second);
	}
	m_csColorMap.Unlock();
	...
	m_dispInfo.hdr.code = CTN_GETDISPINFO;
	m_dispInfo.hdr.hwndFrom = m_hWnd;
	m_dispInfo.hdr.idFrom = GetDlgCtrlID();


	m_dispInfo.item.hItem = hItem;
	m_dispInfo.item.pDC = pDC;
	*pChangeFlag = FALSE;
	m_dispInfo.item.pChangeFlag = pChangeFlag;

	lResult = pWnd->SendMessage(WM_NOTIFY, m_dispInfo.hdr.idFrom, (LPARAM)&m_dispInfo);*/
}
void CColumnTreeCtrl::SetRowBKColor(HTREEITEM hItem,COLORREF color,BOOL bAddFlag)
{
	TREEITEMCOLOR_MAP::iterator iter;
	m_csColorMap.Lock();
	iter = m_colorMap.find(hItem);
...
COLORREF CColumnTreeCtrl::GetRowBKColor(HTREEITEM hItem)
{
	TREEITEMCOLOR_MAP::iterator iter;
	COLORREF color;
	m_csColorMap.Lock();
