...
void CVirtualGridCtrl::SetFont(CFont *pFont)
{
	LOGFONT lf;
	int iRet;
	if (pFont)
	{
		iRet = pFont->GetLogFont(&lf);
	}
	if (iRet!=0)
	{
		m_font.DeleteObject();
...
void CVirtualGridCtrl::MakeCellVisible(const CGridCell &cell, BOOL bPartialOK)
{
	...
	if (m_rows.m_nHeight > 0)  {
		...
		if (m_visSize.m_nRow > 1 && fmod((double)dy,(double) m_rows.m_nHeight) != 0.0)
			y --;
		dy = y;
	}
...
void CVirtualGridCtrl::EditBefore(const CGridCell &cell)
{
	CWnd *pWnd;
	LRESULT lResult;
	...
	m_dispInfo.hdr.code = VGN_EDITBEFORE;
	m_dispInfo.hdr.hwndFrom = m_hWnd;
	m_dispInfo.hdr.idFrom = GetDlgCtrlID();
//	TRACE(_T("ID:%d"),m_dispInfo.hdr.idFrom);
...
void CVirtualGridCtrl::CellClick(const CGridCell &cell, UINT nFlags, int x, int y)
{
	...
	m_dispInfo.hdr.code = VGN_CELLCLICK;
	m_dispInfo.hdr.hwndFrom = m_hWnd;
	m_dispInfo.hdr.idFrom = GetDlgCtrlID();
//	TRACE(_T("ID:%d"),m_dispInfo.hdr.idFrom);
